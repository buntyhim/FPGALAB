...
int main(void)
{
    ...
    //Seven Segment GPIO
  	sevenseg(1,1,1,1,1,1,1);
   	...
   	char A, B, C, D, out, one=0x01;
 	dbg_str("A");dbg_str("\t\t");dbg_str("B");dbg_str("\t\t");dbg_str("C");dbg_str("\t\t");dbg_str("D");dbg_str("\t\t");dbg_str("Out");dbg_str("\n\n");
	while(1)
	{
		disp(i);
		A=(i>>3)&one; B=(i>>2)&one; C=(i>>1)&one; D=(i>>0)&one;	
		out=! ( (A|B)&(C|D) );
 		dbg_hex8(A);dbg_str("\t\t");dbg_hex8(B);dbg_str("\t\t");dbg_hex8(C);dbg_str("\t\t");dbg_hex8(D);dbg_str("\t\t");dbg_hex8(out);dbg_str("\n\n");
		PyHal_GPIO_Set(3,out&one);
...
void disp(int num)
{
	switch(num)
	{
		...
		case 10:		  sevenseg(0,0,0,1,0,0,0);		break;
		case 11:		  sevenseg(1,1,0,0,0,0,0);		break;
		case 12:		  sevenseg(0,1,1,0,0,0,1);		break;
		case 13:		  sevenseg(1,0,0,0,0,1,0);		break;
		case 14:		  sevenseg(0,1,1,0,0,0,0);		break;
		case 15:		  sevenseg(0,1,1,1,0,0,0);		break;
		
		default:		  sevenseg(1,1,1,1,1,1,1);
		break;
	}
}
